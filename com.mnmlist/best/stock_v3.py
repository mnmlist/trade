...
class TestStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if self.position:
            if self.ema15[0] >= self.dataclose or self.ema30[0] >= self.dataclose:
                self.order = self.close()
        else:
            if (self.ema15[-1] < self.ema30[-1] and self.ema15[0] > self.ema30[0]) \
                    or (self.ema10[-1] < self.ema15[-1] and self.ema10[0] > self.ema15[0]):
                self.order = self.buy()
        ...
        # 下降趋势
        # if (self.ema15[-1] > self.ema30[-1] and self.ema15[0] < self.ema30[0]) or self.dataclose > self.ema15[0]:
        #     self.order = self.close()
