...
class TestStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        #         count = count + 1
        # increasing = count > len * 0.7

        if self.position:
            if self.ema15[0] >= self.dataclose or self.ema30[0] >= self.dataclose or self.ema10[0] >= self.dataclose:
            ...
            # percent = math.fabs(self.ema30[0] - self.dataclose) / self.dataclose
            if self.ema15[0] <= self.dataclose and self.ema30[0] <= self.dataclose and self.ema10[0] <= self.dataclose \
                    and self.ema15[0] >= self.ema30[0] and self.ema10[0] >= self.ema15[0]:
                self.order = self.buy()
...
if __name__ == '__main__':
    ...
    data = bt.feeds.GenericCSVData(
        ...
        todate=datetime.datetime(2018, 7, 21),
        dtformat='%Y-%m-%d',
        datetime=1,
        open=2,
        high=3,
