...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self, params=None):
        ...
        self.index_10day_high = bt.ind.Highest(self.index_close, period=10)
        self.cut_price = -1
    ...
    def next(self):
        ...
        if self.position:
            ...
            if self.dataclose < self.cut_price:
                ...
                self.order = self.close()
            elif self.index_10day_high > self.index_close and (self.index_10day_high - self.index_close) / self.index_10day_high > 0.08 \
                    and self.ema15 < self.dataclose:
                # self.global_sell_date = str(self.datas[0].datetime.date(0))
                self.order = self.close()
...
if __name__ == '__main__':
    result_file_name = "result-v16-excellent.csv"
